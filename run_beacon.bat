...
if exist "%~dp0.venv\Scripts\pythonw.exe" (
  start "" "%~dp0.venv\Scripts\pythonw.exe" "%~dp0beacon.py"
  goto :eof
)
if exist "%~dp0.venv\Scripts\python.exe" (
  start "" "%~dp0.venv\Scripts\python.exe" "%~dp0beacon.py"
  goto :eof
)
where pythonw >nul 2>nul && (start "" pythonw "%~dp0beacon.py" & goto :eof)
where python  >nul 2>nul && (start "" python  "%~dp0beacon.py" & goto :eof)
