...
- **Document outline.** A sidebar Outline tab lists the sectioning of the active `.tex` (`\part` through `\subparagraph`) or the classes and functions of the active `.py`, indented by level, and jumps the editor to any entry on click. It follows the active tab and updates as you edit.
...
- **Citation check.** The Tools menu cross-checks the `\cite` keys used in a paper (following `\input` and `\include`) against the entries defined in the workspace `.bib` files, then reports citations with no matching entry and entries that are never cited, so missing and dead references surface before submission.
